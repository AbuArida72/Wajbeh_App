...
import {
  ...
  FlatList, TouchableOpacity, ImageBackground, Image
} from 'react-native';
...
const ALL_BAGS = [
  {
    ...
    logo: 'https://ui-avatars.com/api/?name=Habibah&background=2E7D32&color=fff&size=100',
  },
  {
    id: '2',
    ...
    logo: 'https://ui-avatars.com/api/?name=Sufra&background=2E7D32&color=fff&size=100',
  },
  {
    id: '3',
    ...
    logo: 'https://ui-avatars.com/api/?name=Jafra&background=2E7D32&color=fff&size=100',
  },
  {
    id: '4',
    ...
    logo: 'https://ui-avatars.com/api/?name=Books&background=2E7D32&color=fff&size=100',
  },
  {
    id: '5',
    ...
    logo: 'https://ui-avatars.com/api/?name=Reem&background=2E7D32&color=fff&size=100',
  },
];
...
export default function SearchScreen({ navigation }) {
  ...
  const results = query.length < 2 ? ALL_BAGS : ALL_BAGS.filter(b =>
    ...
    >
      ...
      >
        ...
        <View style={styles.leftBadge}>
          <Text style={styles.leftBadgeText}>{item.quantity_remaining} left</Text>
        </View>
      ...
      <View style={styles.cardBody}>
        <View style={styles.cardRow}>
          <View>
            ...
            <Text style={styles.pickup}>Pick up today {item.pickup_start} - {item.pickup_end}</Text>
            <Text style={styles.area}>{item.area}</Text>
          </View>
          <View style={styles.priceBlock}>
            <Text style={styles.originalPrice}>JD {item.original_value.toFixed(2)}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      {query.length === 0 && (
        <Text style={styles.hint}>Search by restaurant name, area, or category</Text>
      )}
...
const styles = StyleSheet.create({
  ...
  logoContainer: { flexDirection: 'row', alignItems: 'center', padding: 12, gap: 8 },
  logo: { width: 36, height: 36, borderRadius: 18, borderWidth: 2, borderColor: '#FFFFFF' },
  restaurantOverlay: {
    color: '#FFFFFF',
    fontWeight: '700',
    fontSize: 15,
  ...
  bagTitle: { fontSize: 15, fontWeight: '600', color: '#1B5E20', marginBottom: 4 },
  pickup: { fontSize: 12, color: '#5F5E5A', marginBottom: 2 },
  area: { fontSize: 11, color: '#A5C8A5' },
  priceBlock: { alignItems: 'flex-end' },
  originalPrice: { fontSize: 12, color: '#B4B2A9', textDecorationLine: 'line-through', marginBottom: 2 },
